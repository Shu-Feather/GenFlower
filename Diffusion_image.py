...
IMG_WIDTH, IMG_HEIGHT = 32, 32  # Slightly larger images

# Diffusion process settings
num_steps = 1000  # More steps for better diffusion process
beta_min = 1e-4
beta_max = 0.02  # Adjusted based on improved DDPM paper

# Initialize diffusion parameters (cosine schedule - improved DDPM)
...
class TimeEmbedding(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.dim = dim
        inv_freq = torch.exp(torch.arange(0, dim, 2) * (-np.log(10000) / dim))
...
class UNet(nn.Module):
    ...
    def forward(self, x, t):
        ...
        u3 = self.up3(torch.cat([d3, m], dim=1), t)
        u2 = self.up2(torch.cat([d2, u3], dim=1), t)
        u1 = self.up1(torch.cat([d1, u2], dim=1), t)
        
        return self.out_conv(u1)

# Loss function remains the same but with improved noise schedule
...
# Improved sampling with variance control
...
# Sampling loop with EMA model
@torch.no_grad()
def p_theta_sampling_loop(model, shape, n_steps, device):
    model.eval()
    ...
    for t in reversed(range(n_steps)):
        ...
        x_t = p_theta_sampling(model, x_t, t_tensor)
        x_seq.append(x_t)
    
    return x_seq

# Enhanced visualization with EMA model
def visualize_sampling(model, sample_shape, n_steps, epoch, device):
    model.eval()
    x_seq = p_theta_sampling_loop(model, sample_shape, n_steps, device)
...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        ...
        for images, _ in training_dataloader:
            ...
            loss = diffusion_loss_fn(model, images, num_steps)
            optimizer.zero_grad()
            loss.backward()
        ...
        if (epoch + 1) % 10 == 0:
            visualize_sampling(ema_model, sample_shape, num_steps, epoch+1, device)
    ...
    visualize_sampling(model, (16, 3, IMG_WIDTH, IMG_HEIGHT), num_steps, "final", device)